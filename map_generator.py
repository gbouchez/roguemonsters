...
class MapGenerator:
            ...
            for other_room in rooms:
                ...
                for _ in range(number_monsters_to_generate):
                    ...
                    if game_map.can_walk_at(monster_x, monster_y):
                        game_map.add_random_monster(monster_x, monster_y)
                ...
                if num_rooms != 0:
                    ...
                    if randint(0, 1) == 1:
                        continue_carving = self.create_h_tunnel(game_map, begin_x, end_x, begin_y)
                        if continue_carving:
                            self.create_v_tunnel(game_map, begin_y, end_y, end_x)
                    else:
                        continue_carving = self.create_v_tunnel(game_map, begin_y, end_y, begin_x)
                        if continue_carving:
                            self.create_h_tunnel(game_map, begin_x, end_x, end_y)
    ...
    def create_h_tunnel(self, game_map, x1, x2, y):
        first = True
        if x1 <= x2:
            todo = range(x1, x2 + 1)
        else:
    ...
    def create_v_tunnel(self, game_map, y1, y2, x):
        first = True
        if y1 <= y2:
            todo = range(y1, y2 + 1)
        else:
    ...
    def decorate_room(self, game_map, rect):
